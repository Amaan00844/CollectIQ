...
export default function AnalyticsPage() {
  ...
  actions.forEach((a) => {
    tierMap[a.recipient_tier] = (tierMap[a.recipient_tier] || 0) + 1;
  });
  ...
  invoices.forEach((i) => {
    if (i.amount_outstanding > 0)
      custMap[i.customer_name] =
        (custMap[i.customer_name] || 0) + i.amount_outstanding;
  });
  const custData = Object.entries(custMap)
  ...
  const autoActions = actions.filter(
    (a) => a.delivery_mode === "auto_send",
  ).length;
  const humanActions = actions.filter(
    (a) => a.delivery_mode === "human_signoff",
  ).length;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="Analytics"
        description="18-month collections performance overview."
      />
