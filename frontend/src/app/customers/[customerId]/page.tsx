  ...
  return (
    <div className="max-w-3xl space-y-5 animate-fade-in">
      ...
      <PageHeader
        ...
        description={`${cust.industry} · ${cust.contact_email}`}
      />
      ...
      <div className="bg-card border border-border rounded-lg p-5 grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div>
          ...
          <div className="font-medium">{cust.payment_terms_days} days</div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">Total Outstanding</div>
          <div
            className={`font-medium ${cust.total_outstanding > 0 ? "text-amber-600" : "text-emerald-600"}`}
          >
        ...
        <div>
          ...
          <div className="font-medium">{cust.open_invoices}</div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">Total Invoices</div>
          <div className="font-medium">{cust.invoice_count}</div>
        </div>
      </div>
      ...
      {cust.emails.length > 0 && (
        <div className="bg-card border border-border rounded-lg">
          <div className="px-5 py-3 border-b border-border text-sm font-semibold">
            Inbound Emails ({cust.emails.length})
          </div>
          <div className="divide-y divide-border">
            {cust.emails.map((e) => (
              <div
