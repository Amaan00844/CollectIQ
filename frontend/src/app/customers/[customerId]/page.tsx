      ...
      <div className="bg-card border border-border rounded-lg p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="text-sm text-muted-foreground mt-1">{customer.industry} · {customer.email}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-border">
          <div>
            ...
            <div className="text-base font-bold text-foreground mt-1">{formatINRFull(customer.totalOutstanding)}</div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground font-medium">Overdue</div>
            <div className={`text-base font-bold mt-1 ${customer.totalOverdue > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600'}`}>
              {formatINRFull(customer.totalOverdue)}
            </div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground font-medium">Late Payment Rate</div>
            <div className={`text-base font-bold mt-1 ${customer.latePaymentRate >= 60 ? 'text-red-600 dark:text-red-400' : customer.latePaymentRate >= 30 ? 'text-amber-600' : 'text-emerald-600'}`}>
              {customer.latePaymentRate}%
            </div>
          </div>
          <div>
            <div className="text-xs text-muted-foreground font-medium">Avg Payment Delay</div>
            <div className="text-base font-bold text-foreground mt-1">{customer.avgPaymentDelayDays} days</div>
          </div>
        </div>
      </div>
      ...
      {invoices.length > 0 && (
        <div className="bg-card border border-border rounded-lg">
          ...
          <div className="divide-y divide-border">
            {invoices.map(inv => (
              <Link key={inv.id} href={`/invoices/${inv.id}`} className="flex items-center gap-4 px-5 py-3.5 hover:bg-muted/20 transition-colors">
                <span className="font-mono text-xs font-semibold text-primary">{inv.id}</span>
                <span className="text-sm text-foreground flex-1">{inv.description}</span>
                <span className="text-sm font-bold text-foreground">{formatINRFull(inv.amountOutstanding)}</span>
                <StatusBadge status={inv.status} />
                {inv.daysOverdue > 0 && <span className="text-xs text-red-600 dark:text-red-400 font-medium">{inv.daysOverdue}d OD</span>}
              </Link>
            ))}
          </div>
        </div>
            ...
            {actions.slice(0, 5).map(action => (
              <div key={action.id} className="flex items-center gap-3 px-5 py-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-foreground capitalize">{action.action.replace(/_/g, ' ')}</div>
                  <div className="text-xs text-muted-foreground">{action.invoiceId} · {formatDate(action.date)}</div>
                </div>
              </div>
            ))}
          </div>
