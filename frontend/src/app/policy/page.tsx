...
import { useLivePolicy } from "@/lib/live-data";
...
export default function PolicyPage() {
  const { data: policy, isLoading, error } = useLivePolicy();
  if (isLoading)
    return (
  ...
  if (error || !policy)
    return (
      <div className="text-sm text-red-600">
        Unable to load policy. Check the backend URL.
      </div>
    );
  const allStages = [
    ...Object.entries(policy.reminders).map(([k, v]) => ({
      name: k,
      ...v,
      group: "reminder",
    })),
    ...Object.entries(policy.escalation).map(([k, v]) => ({
      name: k,
      ...v,
      group: "escalation",
    })),
    ...
    <div className="max-w-3xl space-y-6 animate-fade-in">
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        <div className="px-5 py-4 flex flex-wrap gap-2">
          {policy.human_review.triggers.map((t) => (
            <span
              key={t}
