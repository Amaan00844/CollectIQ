...
import { useLiveInvoices } from "@/lib/live-data";
...
export default function InvoicesPage() {
  const { data: allInvoices = [], isLoading, error } = useLiveInvoices();
  if (isLoading)
    return (
  ...
  if (error)
    return (
      <div className="text-sm text-red-600">
        Unable to load invoices. Check the backend URL.
      </div>
    );
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <PageHeader
        title="All Invoices"
        description="Track and manage all invoice collection activities."
      />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th
                  className="px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide cursor-pointer"
                ...
                >
                  <div className="flex items-center gap-1">
                    Days OD <ArrowUpDown className="w-3 h-3" />
                  </div>
                </th>
                <th
                  className="px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide cursor-pointer"
